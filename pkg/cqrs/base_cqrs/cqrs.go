...
// SELECT COUNT(*) instead of fetching and deserializing every row.
...
func (w wrapper) GetSpanRuns(ctx context.Context, opt cqrs.GetTraceRunOpt) ([]*cqrs.TraceRun, error) {
	...
	if w.isPostgres() {
		// PostgreSQL: use CTE + DISTINCT ON to avoid expensive correlated subquery.
		// The original correlated subquery for status ran once per GROUP, which is O(N)
		// subqueries for N groups — extremely slow with large datasets (>5min for 50K runs).
		// This CTE approach pre-filters rows once, aggregates, and uses DISTINCT ON for
		// the latest status lookup.
		var err error
...
func (w wrapper) buildPostgresSpanRunsQuery(
	...
	selectCols := []interface{}{
		...
		sq.L(`(SELECT s2.status FROM spans s2
			WHERE s2.run_id = spans.run_id AND s2.dynamic_span_id = spans.dynamic_span_id
			ORDER BY s2.end_time DESC NULLS LAST LIMIT 1)`).As("status"),
		// Use direct column reference instead of adapter.eventIdsExpr which uses MAX() aggregate.
		// No GROUP BY in this query — each root span row has its own event_ids.
